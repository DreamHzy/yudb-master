...
public interface PaymentOrderExamineMapper extends MyBaseMapper<PaymentOrderExamine> {
    ...
    @Select("" +
            ...
            "  p.CREATE_TIME, " +
            "  ( SELECT pd.EXPIRE_TIME FROM payment_order_master pd WHERE pd.ID = p.PAYMENT_ORDER_MASTER_ID ) expireTime, " +
            "  p.MSG 'reviewMsg', " +
            "  ms.`STATUS`, " +
            "  t1.`STATUS` EXAMINE_ONE_STATUS, " +
            "  t2.`STATUS` EXAMINE_TWO_STATUS " +
            "FROM " +
            "  payment_order_examine p " +
            "  INNER JOIN merchant_store ms ON p.MERCHANT_STORE_ID = ms.ID " +
            "  LEFT JOIN ( SELECT pd.PAYMENT_ORDER_EXAMINE_ID, pd.`STATUS` FROM payment_order_examine_deail pd WHERE EXAMINE = 1 AND pd.DELETED = 0 ) t1 ON t1.PAYMENT_ORDER_EXAMINE_ID = p.id " +
            "  LEFT JOIN ( SELECT pd.PAYMENT_ORDER_EXAMINE_ID, pd.`STATUS` FROM payment_order_examine_deail pd WHERE EXAMINE = 2 AND pd.DELETED = 0 ) t2 ON t2.PAYMENT_ORDER_EXAMINE_ID = p.id " +
            "WHERE " +
            " 1=1 " +
            "  AND p.EXAMINE != - 1  " +
            "  AND p.DELETED = 0  " +
    ...
    @Select("<script>" +
            ...
            " t1.CREATE_TIME, " +
            " t2.EXPIRE_TIME, " +
            " ( SELECT REGION FROM merchant_store_mapping_area WHERE PROVINCE = t4.PROVINCE AND CITY = t4.CITY AND AREA = t4.AREA ) REGION, " +
            " p3.APPROVE_NAME  " +
            "FROM " +
            " payment_order_examine t1 " +
            " LEFT JOIN payment_order_master t2 ON t1.PAYMENT_ORDER_MASTER_ID = t2.ID" +
            " LEFT JOIN ( " +
            " SELECT " +
            "  p1.PAYMENT_ORDER_EXAMINE_ID, " +
            "  p1.APPROVE_NAME  " +
            " FROM " +
            "  payment_order_examine_deail p1 " +
            "  INNER JOIN ( SELECT PAYMENT_ORDER_EXAMINE_ID, MAX( ID ) ID FROM payment_order_examine_deail WHERE DELETED = 0 GROUP BY PAYMENT_ORDER_EXAMINE_ID ) p2  " +
            " WHERE " +
            "  p1.PAYMENT_ORDER_EXAMINE_ID = p2.PAYMENT_ORDER_EXAMINE_ID  " +
            "  AND p1.ID = p2.ID  " +
            " ) p3 ON t1.ID = p3.PAYMENT_ORDER_EXAMINE_ID " +
            " LEFT JOIN merchant_store t4 ON t2.MERCHANT_STORE_UID = t4.UID " +
            "<where>" +
            " t1.DELETED = 0 " +
            "<if test='applyName != null'> AND t1.APPLY_NAME LIKE CONCAT('%',#{applyName},'%') </if>" +
            "<if test='merchantStoreUid != null'> AND t2.MERCHANT_STORE_UID LIKE CONCAT('%',#{merchantStoreUid},'%') </if>" +
            ...
            "<if test='region != null'> AND ( SELECT REGION FROM merchant_store_mapping_area WHERE PROVINCE = t4.PROVINCE AND CITY = t4.CITY AND AREA = t4.AREA ) = #{region} </if>" +
            "</where>" +
    ...
    @Select("<script>" +
            ...
            " payment_order_examine t1 " +
            " LEFT JOIN payment_order_master t2 ON t1.PAYMENT_ORDER_MASTER_ID = t2.ID " +
            " INNER JOIN payment_order_examine_deail t3 ON t1.ID = t3.PAYMENT_ORDER_EXAMINE_ID  " +
            " LEFT JOIN merchant_store t4 ON t2.MERCHANT_STORE_UID = t4.UID " +
            "WHERE " +
            ...
            "  t3.CREATE_TIME, " +
            "  t5.EXPIRE_TIME,  " +
            " ( SELECT REGION FROM merchant_store_mapping_area WHERE PROVINCE = t6.PROVINCE AND CITY = t6.CITY AND AREA = t6.AREA ) REGION " +
            " FROM " +
            "  approve_process_step t1 " +
            "  INNER JOIN approve_process t2 ON t1.APPROVE_ID = t2.ID " +
            "  INNER JOIN payment_order_examine t3 ON t2.EXAMINE_TYPE = t3.EXAMINE_TYPE " +
            "  INNER JOIN payment_order_examine_deail t4 ON t3.ID = t4.PAYMENT_ORDER_EXAMINE_ID  " +
            "  AND t1.STEP = t4.STEP  " +
            "  AND t1.TYPE = t4.TYPE " +
            "  INNER JOIN payment_order_master t5 ON t3.PAYMENT_ORDER_MASTER_ID = t5.ID  " +
            "  LEFT JOIN merchant_store t6 ON t5.MERCHANT_STORE_UID = t6.UID " +
            " WHERE " +
            "  t1.`STATUS` = 1  " +
            "  AND t2.`STATUS` = 1  " +
            "  AND t2.TYPE = 1  " +
            ...
            "<if test='merchantStoreUid != null'> AND p.MERCHANT_STORE_UID LIKE CONCAT('%',#{merchantStoreUid},'%') </if>" +
            ...
            "<if test='region != null'> AND ( SELECT REGION FROM merchant_store_mapping_area WHERE PROVINCE = t6.PROVINCE AND CITY = t6.CITY AND AREA = t6.AREA ) = #{region} </if>" +
            "</where>" +
...
//            " t1.`STATUS`, " +
    ...
    @Select("SELECT " +
            ...
            " t1.EXAMINE_TIME  " +
            "FROM " +
            " payment_order_examine_deail t1 " +
    ...
    @Select("SELECT " +
            ...
            " URL  " +
            "FROM " +
            " payment_order_examine_deail_file  " +
            "WHERE " +
            " PAYMENT_ORDER_EXAMINE_DEAIL_ID = #{id}  " +
            " AND TYPE = #{type}  " +
            " AND `STATUS` = 1  " +
            " AND DELETED = 0")
