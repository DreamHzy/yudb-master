...
@Service
public class StoreRenewalServiceImpl implements StoreRenewalService {
    ...
    @Override
    public ApiRes<PageWrap<StoreRenewalPageVo>> findPage(PageWrap<StoreReviewPageDto> pageWrap) {
        ...
        List<StoreRenewalPageVo> storeRenewalPageVos = merchantStoreExamineMapper.storeRenewalPageVos(storeReviewPageDto);
        storeRenewalPageVos.stream().forEach(
                storeRenewalPageVo -> {
                    switch (storeRenewalPageVo.getExamineOneStatus()) {
                        case "1":
                            storeRenewalPageVo.setExamineOneStatus("待审核");
                            storeRenewalPageVo.setExamineTwoStatus("");
                            break;
                        case "2":
                            storeRenewalPageVo.setExamineOneStatus("审核通过");
                            break;
                        case "3":
                            storeRenewalPageVo.setExamineOneStatus("拒绝");
                            break;
                        default:
                            storeRenewalPageVo.setExamineOneStatus("");
                    }
                    if (!StringUtils.isEmpty(storeRenewalPageVo.getExamineTwoStatus())) {
                        switch (storeRenewalPageVo.getExamineTwoStatus()) {
                            case "1":
                                storeRenewalPageVo.setExamineTwoStatus("待审核");
                                break;
                            case "2":
                                storeRenewalPageVo.setExamineTwoStatus("审核通过");
                                break;
                            default:
                                storeRenewalPageVo.setExamineTwoStatus("拒绝");
                        }
                    } else {
                        storeRenewalPageVo.setExamineTwoStatus("");
        ...
        return ApiRes.response(CommonConstant.SUCCESS_CODE, "查询成功", PageData.from(new PageInfo<>(storeRenewalPageVos)));
    }
    ...
    public ApiRes two(StoreReviewDto storeReviewDto) {
        ...
        if ("2".equals(status)) {//通过该后需要对门店的续约时间进行操作/管理费等金额进行操作
            ...
            merchantStore.setExpireTime(merchantStoreExamine.getNewTime());
            merchantStore.setManagementExpense(merchantStoreExamine.getNewMoney());
            merchantStore.setAlreadyManagementExpense(BigDecimal.ZERO);
    ...
    @Override
    public ApiRes<StoreRenewalDetailVo> detail(StoreReviewDetailDto storeReviewDetailDto) {
        ...
        StoreRenewalDetailVo storeRenewalDetailVo = new StoreRenewalDetailVo();
        storeRenewalDetailVo.setUid(merchantStoreExamine.getMerchantStoreUid());
        storeRenewalDetailVo.setOldMoney(merchantStoreExamine.getOldMoney() + "");
        storeRenewalDetailVo.setNewMoney(merchantStoreExamine.getNewMoney() + "");
        ...
        storeRenewalDetailVo.setExamineDetailVoList(examineDetailVoList);
        ...
        storeRenewalDetailVo.setExamineFileVoList(examineFileVoList);
        return ApiRes.response(CommonConstant.SUCCESS_CODE, "查询成功", storeRenewalDetailVo);
    }
}
